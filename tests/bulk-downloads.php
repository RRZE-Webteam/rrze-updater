...
foreach ([$github, $lab] as $connector) {
    ...
    foreach (['plugin' => [Bulk_Plugin_Upgrader_Skin::class, WP_Ajax_Upgrader_Skin::class, Automatic_Upgrader_Skin::class],
        ...
        foreach ($skins as $skinClass) {
            ...
            $path = $managed->upgraderPreDownloadFilter(false, $connector->downloadRepoZip('package', 'v1'), $upgrader, $extra);
        ...
        $path = $managed->upgraderPreDownloadFilter(false, $connector->downloadRepoZip('package', 'v1'), $upgrader, $extra + ['type' => $type]);
        check(is_string($path) && is_file($path), 'Single-update payloads still authenticate.');
        wp_delete_file($path);
        check(is_wp_error($managed->upgraderPreDownloadFilter(false, 'https://unrelated.example/archive.zip', $upgrader, $extra)), 'Reject unrelated package URLs for managed update targets.');
        check($managed->upgraderPreDownloadFilter(false, $connector->downloadRepoZip('package', 'v1'), $upgrader,
        ...
        check(is_wp_error($managed->upgraderPreDownloadFilter(false, $connector->downloadRepoZip('package', 'v1'), $upgrader, $extra)), 'Authenticated bulk download failure never falls back to a public request.');
    ...
    check(is_wp_error($managed->upgraderPreDownloadFilter(false, $connector->downloadRepoZip('package', 'v1'), $upgrader, ['plugin' => 'package/main.php'])), 'Invalid bulk plugin repository blocks the download.');
