...
foreach ([Bulk_Plugin_Upgrader_Skin::class, WP_Ajax_Upgrader_Skin::class, Plugin_Upgrader_Skin::class, Automatic_Upgrader_Skin::class] as $skinClass) {
    ...
    $result = $managed->upgraderSourceSelectionFilter('/tmp/archive/', '/tmp/work/', $upgrader, ['plugin' => 'target/target.php']);
    ...
    check($managed->upgraderSourceSelectionFilter('/tmp/archive/', '/tmp/work/', $upgrader, ['plugin' => 'first/first.php']) === '/tmp/archive/', 'Unmanaged plugin with the same display name is untouched.');
    check($managed->upgraderSourceSelectionFilter('/tmp/archive/', '/tmp/work/', $upgrader, []) === '/tmp/archive/', 'A display name alone cannot choose a target.');
...
foreach ([Bulk_Theme_Upgrader_Skin::class, WP_Ajax_Upgrader_Skin::class, Theme_Upgrader_Skin::class, Automatic_Upgrader_Skin::class] as $skinClass) {
    ...
    $upgrader = new WP_Upgrader($skin);
    check($managed->upgraderSourceSelectionFilter('/tmp/archive/', '/tmp/work/', $upgrader, ['theme' => 'theme-target']) === '/tmp/work/theme-target/', 'Managed theme uses its exact stylesheet identifier.');
    check($managed->upgraderSourceSelectionFilter('/tmp/archive/', '/tmp/work/', $upgrader, ['theme' => 'unmanaged']) === '/tmp/archive/', 'Unmanaged theme is untouched.');
    $moves = count($wp_filesystem->moves);
    check($managed->upgraderSourceSelectionFilter('/tmp/work/theme-target/', '/tmp/work/', $upgrader, ['theme' => 'theme-target']) === '/tmp/work/theme-target/'
...
foreach ([PluginUpgraderSkin::class => $plugin, ThemeUpgraderSkin::class => $theme] as $skinClass => $extension) {
    ...
    $upgrader = new WP_Upgrader($skin);
    check($managed->upgraderSourceSelectionFilter('/tmp/archive/', '/tmp/work/', $upgrader, []) === '/tmp/archive/', 'A legacy skin without a verified package cannot rename a source.');
    check($managed->upgraderSourceSelectionFilter('/tmp/archive/', '/tmp/work/', $upgrader, ['theme' => 'unmanaged-parent']) === '/tmp/archive/', 'Explicit core identifiers take priority over a reused custom skin.');
}
$wp_filesystem->fail = true;
check(is_wp_error($managed->upgraderSourceSelectionFilter('/tmp/archive/', '/tmp/work/', $upgrader, ['plugin' => 'target/target.php'])), 'Filesystem failure blocks the update.');
$error = new WP_Error('invalid_archive', 'Invalid archive');
check($managed->upgraderSourceSelectionFilter($error, '/tmp/work/', $upgrader, ['plugin' => 'target/target.php']) === $error, 'Preserve an earlier source-validation failure.');
